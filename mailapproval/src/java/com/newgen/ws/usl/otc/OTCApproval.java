...
public class OTCApproval {
    ...
    public OTCOutputXml postXml(OTCInputXml ix) throws SQLException, ClassNotFoundException, IOException {
        CommonMethods call = new CommonMethods();
        OTCOutputXml xml = new OTCOutputXml();
        ...
        if (call.ReadIni()) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("approve")) {
                    ...
                    String searchWorkstep = "Select CURR_WORKSTEP+'`'+PREV_WORKSTEP+'`'+CONVERT(NVARCHAR,SA_Curr_Loop)+'`'+CONVERT(NVARCHAR,SA_Total) from EXT_CustomerDispute with(nolock) where PROCESSINSTID='" + ProcessInstId + "'";
                    ...
                    if (WorkstepData.length > 2) {
                        ...
                        CurrentLoop = WorkstepData[2];
                        TotalLoop = WorkstepData[3];

                        CommonMethods.GenLog("Inside OTC Approval", "T");
                        if (call.ReassignWorkitem(ProcessInstId, CommonMethods.UserName, AssignedUser)) {
//                            String Query = "";
                            int inc = Integer.parseInt(CurrentLoop) + 1;
                            String Query = "UPDATE EXT_CustomerDispute SET ACTIONITEM='ACCEPT',SA_Curr_Loop='" + inc + "',Comments='" + Comments + "',APPROVALEDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                            ...
                            if (Flag) {
                                ...
                                if (inc <= Integer.parseInt(TotalLoop)) {
                                    CommonMethods.InsertCommentsHistory(ProcessInstId, "CreditDebitNote_Approvals", "CreditDebitNote_Approvals", Curr_Workstep, Prev_Workstep, "CreditDebitNote_Approvals", UserName, Comments);
                                } else {
                                    CommonMethods.InsertCommentsHistory(ProcessInstId, "CreditDebitNote_Posting", "CreditDebitNote_Posting", Curr_Workstep, "CreditDebitNote_Approvals", "CreditDebitNote_Approvals", UserName, Comments);
                                }
                    ...
                    //fetch Current & Previous Workstep
                    String searchWorkstep = "Select CURR_WORKSTEP+'`'+PREV_WORKSTEP+'`'+CONVERT(NVARCHAR,SA_Curr_Loop) from EXT_CustomerDispute with(nolock) where PROCESSINSTID='" + ProcessInstId + "'";
                    CommonMethods.GenLog("searchWorkstep-->> " + searchWorkstep, "T");
                    String sOutput = CommonMethods.selectQuery(searchWorkstep, 1);
                    xmlResponse = new WFXmlResponse(sOutput);
                    String Workstep = "";
                    ...
                    String[] WorkstepData = Workstep.split("`");
                    String Prev_Workstep = "", Curr_Workstep = "", CurrentLoop = "";
                    if (WorkstepData.length > 2) {
                        Curr_Workstep = WorkstepData[0];
                        Prev_Workstep = WorkstepData[1];
                        CurrentLoop = WorkstepData[2];

                        CommonMethods.GenLog("Inside OTC Reject", "T");
                        if (call.ReassignWorkitem(ProcessInstId, CommonMethods.UserName, AssignedUser)) {
                            //decrement Currentloop
                            int dec = Integer.parseInt(CurrentLoop) - 1;
                            String Query = "UPDATE EXT_CustomerDispute SET ACTIONITEM='REJECT',SA_Curr_Loop='" + dec + "',Comments='" + Comments + "',REJECTSDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                            CommonMethods.GenLog("EXTERNAL Update-->> " + Query, "T");
                            CommonMethods.ExecuteUpdateQuery(Query);
                            ...
                            if (Flag) {
                                ...
                                if (dec == 0) {
                                    CommonMethods.InsertCommentsHistory(ProcessInstId, "CreditDebitNote_Requestor", "CreditDebitNote_Requestor", Curr_Workstep, Prev_Workstep, Curr_Workstep, UserName, Comments);
                                } else {
                                    CommonMethods.InsertCommentsHistory(ProcessInstId, "CreditDebitNote_Approvals", "CreditDebitNote_Approvals", Curr_Workstep, Prev_Workstep, Curr_Workstep, UserName, Comments);
                                }
