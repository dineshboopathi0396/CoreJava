...
public class VMApproval {
    ...
    public VMOutputXml postXml(VMInputXml ix) throws SQLException, ClassNotFoundException, IOException {
        CommonMethods call = new CommonMethods();
        VMOutputXml xml = new VMOutputXml();
        ...
        if (call.ReadIni()) {
            ...
            CommonMethods.GenLog("*******Inside VMApproval>>","T");
            ...
            try {
                ...
                String sQuery_total="SELECT SA_CurrentLoop,SA_Total FROM EXT_VM WITH (nolock) WHERE PROCESSINSTID='"+ProcessInstId+"'";
                ...
                if(checkflag.equalsIgnoreCase("Y"))
                {
                       if (Action.equalsIgnoreCase("approve")) {
                                String searchWorkstep = "Select CURR_WORKSTEP from EXT_VM with(nolock) where PROCESSINSTID='" + ProcessInstId + "'";
                                ...
                                String searchPREV_Workstep = "Select PREV_WORKSTEP from EXT_VM with(nolock) where PROCESSINSTID='" + ProcessInstId + "'";
                                ...
                                if (Curr_Workstep.equalsIgnoreCase("Approver")) {
                                    CommonMethods.GenLog("Inside VM Approver","T");
                                    if (call.ReassignWorkitem(ProcessInstId, SystemUser,UserName)){// AssignedUser)) {
                                        String Query = "UPDATE EXT_VM SET ACTIONITEM='approve',Comments='" + Comments + "',APPROVEREDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                                        ...
                                        if (Flag) {
                                            ...
                                            CommonMethods.mailTrigger_VM(ProcessInstId);    
                                            }
                                ...
                                if (Curr_Workstep.equalsIgnoreCase("Approver")) {
                                    CommonMethods.GenLog("Inside VM Approval","T");
                                    if (call.ReassignWorkitem(ProcessInstId, SystemUser, UserName)){//AssignedUser)) {
                                        String Query = "UPDATE EXT_VM SET ACTIONITEM='Reject',Comments='" + Comments + "',APPROVEREDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                                        CommonMethods.GenLog("EXTERNAL Update-->> " + Query,"T");
                                        call.ExecuteUpdateQuery(Query);
                                        boolean Flag = call.CompleteWorkitem(ProcessInstId, "1");
                                        CommonMethods.GenLog("CompleteWorkitem Flag-->> " + Flag,"T");
                ...
                CommonMethods.GenLog("VMApproval Exception-->> " + e,"T");
                Action="Error";
                String UpdateHistory = "Insert INTO WS_MAILAPPROVAL_HISTORY (PROCESS,PROCESSINSTID,USERNAME,COMPLETIONDATE,STATUS,ACTION,Mode,SequenceNo) values ('" + Process + "','" + ProcessInstId + "','" + tempusername + "',getDate(),'" + sMessage + "','"+Action+"','" + mode + "','"+seqid+"')";
                call.ExecuteUpdateQuery(UpdateHistory);                
            }
