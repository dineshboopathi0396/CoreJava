...
public class RTRApproval {
    ...
    public RTROutputXml postXml(RTRInputXml ix) throws SQLException, ClassNotFoundException, IOException {
        CommonMethods call = new CommonMethods();
        RTROutputXml xml = new RTROutputXml();
        ...
        if (call.ReadIni()) {
            ...
            try {
                ...
                if (Action.equalsIgnoreCase("approve")) {
                    String searchWorkstep = "Select isnull(CURR_WORKSTEP,'')+'`'+isnull(PREV_WORKSTEP,'') from EXT_RTR with(nolock) where PROCESSINSTID='" + ProcessInstId + "'";
                    ...
                    CommonMethods.GenLog("Inside RTR RO_Approval", "T");
                    if (call.ReassignWorkitem(ProcessInstId, CommonMethods.UserName, AssignedUser)) {
                        String sQry = "Select isnull(JournalType,'')+'`'+isnull(CURR_APPROVERCOUNT,'')+'`'+isnull(SA_CurrentLoop,'') FROM EXT_RTR WITH(NOLOCK) WHERE PROCESSINSTID='" + ProcessInstId + "'";
                        ...
                        String sComplianceQry = "Select FLAG_COMPLIANCE FROM EXT_RTR WITH(NOLOCK) WHERE PROCESSINSTID='" + ProcessInstId + "'";
                        ...
                        if (workItemFetch.length > 1) {
                            ...
                            String Query = "UPDATE EXT_RTR SET ACTIONITEM='Approve',PostingFlag='" + PostingFlag + "',Comments='" + Comments + "',APPROVEREDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                            ...
                            if (Flag) {
                                ...
                                if (approverCount.equals("1")) {
                                    ...
                                    if (currentLoop.equals("1")) {
                                        String fetchApproval = "SELECT isnull(APPROVER2,'')+'`'+isnull(APPROVER2EMAIL,'') from EXT_RTR with(nolock) WHERE PROCESSINSTID='" + ProcessInstId + "'";
                                        ...
                                        if (approvalArr.length > 0) {
                                            ...
                                            try {
                                                CommonMethods.mailTrigger_RTR(ProcessInstId, Approver2Mail, AssignedUser, Approver2, Comments);
                                            } catch (Exception e) {
                                                CommonMethods.GenLog("Exception in RTR mailtrigger" + e, "E");
                                            }
                                        }
                            ...
                            sMainCode = "63";
                            sMessage = "Error in fetching data from EXT_RTR";
                            CommonMethods.GenLog("sMessage-->> " + sMessage, "T");
                            sType = "E";
                        }
                    } else {
                    ...
                    CommonMethods.GenLog("Inside RTR Reject", "T");
                    if (call.ReassignWorkitem(ProcessInstId, CommonMethods.UserName, AssignedUser)) {
                        String Query = "UPDATE EXT_RTR SET ACTIONITEM='Reject',Comments='" + Comments + "',REJECTSDATE=getDate() where PROCESSINSTID='" + ProcessInstId + "'";
                        CommonMethods.GenLog("EXTERNAL Update-->> " + Query, "T");
                        CommonMethods.ExecuteUpdateQuery(Query);
                        boolean Flag = call.CompleteWorkitem(ProcessInstId, "1");
                        CommonMethods.GenLog("CompleteWorkitem Flag-->> " + Flag, "T");
                ...
                CommonMethods.GenLog("RTRApproval Exception-->> " + e, "E");
            }
